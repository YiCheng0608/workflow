---
name: orchestrator
description: 與任務類型無關的通用任務引擎編排器。讀磁碟上的 manifest 當唯一事實來源,呼叫 cli.js 取得「下一步該做什麼」,把實際工作交給 worker subagent,worker 產出後依 review map 決定 full / focused / deferred 審查,能機器驗的節點在 runtime-preflight 準備依賴環境後實跑 test,再把結果寫回 manifest,直到完成或被安全護欄擋下。任務的拆解、驗證地圖與 review map 不預先寫死,而是在 intake 階段針對該次需求現生、過人類 gate 後凍結。Use when the user wants to drive an arbitrary requirement end to end through a planned, verified task list (intake → human gate → worker/review-map → runtime-preflight + machine-verify where possible → done), or to resume a half-finished run; do not use to do a single task's work directly (delegate that to a worker) or to make routing decisions by hand.
---
...
測試前的 runtime 依賴準備由 `runtime-preflight` skill 承擔。它不是 manifest 節點,也不是引擎 routing 的一部分;orchestrator 在委派 test worker 前使用它的規則,讓測試 worker 在 worktree 內建立 / 修復自己的可變依賴投影,並只共享安全的 cache / store。
...
- **worker 產出依人類 gate 核准的 review map 審查**(見「reviewer 迴圈」)。`full` / `focused` 都要派 reviewer;`defer-until-signal` 只是在 produce 前合法略過 reviewer,後續仍必跑 machine test。reviewer 全程不碰 manifest / `result.json` / routing,只對著需求原文挑錯、不代改;通過或合法 defer 後你才記 `ok:true`,審出做歪才記 `ok:false` 回頭重做。
...
- **test-runner worker 只能經 `cli.js test` 驅動**。任何 `manifest.tests[*].runner` 指到的測試只能由 `next` / `next-all` 吐出的 `test` action 觸發、結果只能經 `cli.js test` 記回——這樣 `environment` 失敗才會觸發 `m.block`→`clarify` 停下問人。測試前需要依賴環境時,先依 `runtime-preflight` 準備 worktree 內的可變投影與共享 cache / store,再跑真 test;preflight 失敗也要以 test result 的 `altitude:"environment"` 記回。絕不可為了「補跑一下」直接呼叫測試 worker、把它寫出的 .md 摘要當數,那會繞過引擎悄悄跑完。manifest 沒有對應 test 節點時要補做機器驗收,走「補做機器驗收」。
...
| `{"type":"test","test":"test-x","spec":"spec-2"}` | 該驗這個 test | 先依 `runtime-preflight` 準備 runtime,再依 `test.runner` 委派對應測試 worker,然後 `cli.js test` |
...
`next` / `next-all` 回傳的 `produce` / `test` action 都附一個 `brief` 欄位:引擎從 manifest 機械算出的派工輸入(`skill` 角色、上游 `depends_on_outputs`、`gate_outputs`(intake 文件路徑)、`last_failure` / `fix_target`、ownership 約束、`review.required_depth`、`env`)。委派時機械輸入以 `brief` 為準,不要自己讀 manifest 重新組裝;語意(需求原文、任務描述)仍讀 `orchestrator/requirement.md` 與 intake 文件。
...
1. **準備 manifest**(intake —— 整條流程唯一的動態步驟,規劃完就凍結):
   ...
   - 若不存在,且使用者給的是真實需求 → 先把使用者原始需求原封不動寫成 `orchestrator/requirement.md`,再進「intake:動態規劃」,產出 intake 文件與 draft manifest,再用 `cli.js produce` 記回 intake spec 觸發人類 gate;使用者同意後才凍結並放行下游。不要為同一份 draft 再跑第二次 intake。
   - 一律對 `$MANIFEST` 操作,不要動到 example 檔。
2. **問這一輪能跑什麼**(依執行模型擇一):平行用 `cli.js next-all`(一批互相獨立的 action),序列用 `cli.js next`(單一)。解析印出的 JSON。
3. **派工**:
   - `produce` → 依 action 的 `brief`(引擎機械算出的 `skill` 角色、上游 outputs、guards、required review depth、env)派一個 worker 子代理;語意輸入取自 `orchestrator/requirement.md` 與 intake 產出裡這個任務的描述(子代理 context 隔離,要把檔案路徑與語意標籤明確交給它)。若 spec 有 `tier:"high"` / `"low"`,依本 host 的對應選擇子代理模型 / 設定;沒有對應能力就忽略 tier。若這是重做(該 spec 的 `last_failure` 非 `null`)→ 務必把 `last_failure`(上次錯在哪)與 `fix_target`(`code`=修實作 / `spec`=改規格語意)一起交給 worker,讓它針對性修而非盲目重跑。並要求 worker:過程產物(`.md` 摘要等)寫在 `orchestrator/` 底下(避免被當成未提交的工作),且回報簡短 handoff summary(改了哪些檔、如何驗、風險點)。worker 產出後依 `planning.review_map` 決定 produce 前 reviewer 深度(見「reviewer 迴圈」),收斂版或合法 defer 才記回。
   - `test` → 先在目標 worktree 依 `runtime-preflight` 規則準備測試所需 runtime:偵測 lockfile / manifest、計算包含 lockfile / runtime / package manager / OS-arch / native ABI / install flags 的 fingerprint、保留 worktree 內可變投影、只共享 package-manager-owned cache / store。preflight 成功後,把 `manifest.tests[test]` 對應的測試委派給子代理跑(對 `verifies` 的那個 spec 的產物),並要求它把 preflight 狀態納入 evidence。preflight 或測試環境失敗時,test worker 回 `{ "pass": false, "altitude": "environment", "reason": "...", "evidence": {...} }`,不可歸咎到 spec。
...
3. **委派輸入要完整**:每次委派都明確提供 `orchestrator/requirement.md`、任務描述、上游 `outputs` 檔案路徑與 `env` 事實;produce 站也要要求 worker 回報 handoff summary,供 focused reviewer 當索引。
...
worker 產出不是無條件直接記回:worker 產出後,依 human gate 核准的 `planning.review_map` 選審查深度。`review_map` 未列的 spec(或整份缺失)一律預設 `full`,讓 cost-aware 降級只能明確 opt-in、不能因漏記 fail-open。引擎對此無知(只認得 `cli.js produce` 記回的結果);reviewer 全程不碰 manifest / `result.json` / routing,只挑戰、不代改。
...
1. **派 intake 子代理**產出分析書 + 任務清單 + **驗證地圖** + **review map**(下游 worker 任務逐一列驗證方式、風險、拆分理由、review depth 與升級條件)。高槓桿,intake 產出先過一輪「盡力挑漏」的對抗式 reviewer,結論寫成 `orchestrator/review-<intake spec id>.md`(記回時作 `review.record`);這一輪不靠 review map。
...
> **`result.json` 是一次性參數,用完即丟。** 它唯一的作用是把 worker 這一輪的結果搬進 manifest(`cli.js` 讀它 → `applyProduce`/`applyTestResult` 折進 manifest → 存檔);搬完就沒用了,`decide.js` 從頭到尾只讀 manifest。所以用單一 scratch 路徑每回合覆寫(例如 `/tmp/wf-result.json`)即可,不要 per-spec / per-test 留一堆。`produce` 成功記回必帶 `review` 欄位,所以 result.json 實務上不可省略;`review_gate` spec 與任何下游需要讀 outputs 的 spec 必須明確記 outputs;`test` 的必填。
...
- 每個 worker 委派都要求 handoff summary(改了哪些檔、如何驗、風險點);每個 worker 產出都依 `planning.review_map` 完成 produce 前 `full` / `focused` reviewer 或合法 `defer-until-signal`,reviewer 結論落盤並經 result.json 的 `review` 欄位記回;produce 後 test 失敗 / evidence 不足已經由 `cli.js test` 回寫,且重做下一輪會升級為 `full`
...
- 委派 `test` 時已先依 `runtime-preflight` 準備 runtime 依賴環境,再依 `test.runner` 選對測試 worker;能機器驗的節點都實跑(非以 reviewer 主觀判定取代)
