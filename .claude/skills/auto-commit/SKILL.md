---
name: auto-commit
description: flow 無關的 commit 站。依 commit context（commitType、scope 等）與挑檔規則，在當前 repo/worktree 產生本地 Conventional Commit，可選把過程紀錄寫到孤兒 journal 分支。支援 default / initial / declared 三種挑檔模式。全程 local，不建分支、不切分支、不 push。Use when deliverables need a local commit; do not use to create branches, run a pipeline, push, or open PRs.
---
...
本站依 commit context 與挑檔規則,在**當前分支**產生本地 commit；若有過程紀錄清單,另寫到 journal 分支。它只做兩件事,且都**全程 local**:

1. **commit 交付物**:依模式挑出交付檔並 `git add`,以 **Conventional Commit** 格式 commit 到**當前分支**。
2. **寫入過程紀錄**:把過程紀錄檔清單寫到**孤兒分支 `journal/<scope>`**。它與交付分支毫無交集、**永不 merge 回 dev**;交付分支保持純交付物,過程紀錄仍可回看。

## 前提（由呼叫端 / flow 保證,本站依賴而不重複）

- **分支已準備好**:本站**不建分支、不切分支、不加 collision 尾碼**。呼叫端須在進入本站前讓 repo 位於應提交的分支。
- **worktree 範圍可判定**:預設模式假設 worktree 從乾淨開始；declared 模式則以呼叫端提供的聲明交付清單為權威。
...
- **過程紀錄清單**(選填):要寫到 journal 的摘要 / 審計檔路徑。
...
- **short description**:imperative mood(英文),整行含前綴 ≤72 字元;描述本次主要交付內容。優先用呼叫端提供的一句描述；缺席時才從輸入摘要或交付檔路徑推斷。

## 執行前判斷與例外處理

**硬拒絕**(不執行 commit、回報說明文字、不寫任何檔):
- worktree 挑不到任何程式碼或測試檔變更(篩掉 `.md` 後候選為空)→ 回報「no code/test changes found to commit」
- `git status --porcelain` 顯示有**已 staged** 的變更 → 警告並停下,不執行 `git add`;人工確認後再重跑(auto-commit 前不應已有 staged 變更)

> **硬拒絕怎麼處置**:auto-commit 沒有 test。硬拒絕時不要記成成功;把說明交給呼叫端。合法出口只有兩種:**修好原因 → 重跑**；或**明示「本來無需 commit」→ 呼叫端可視為成功**。

## 模式:initial(`commitMode === "initial"`)

綠地前提不同,以下幾點**覆寫**預設行為;未提到的(staged 先擋、journal 寫入、硬拒絕處置、local 界線、輸出結構)**全部照舊**:

- **前提差異**:repo 位於初始分支(`main`),這將是**第一個 commit**。「worktree 從乾淨開始」改由「初始 repo 中一切 untracked 皆為本次鷹架交付物」保證。
- **挑檔規則(關鍵差異)**:收**全部鷹架交付物**——code、test、設定檔、lockfile、**README / AGENTS.md / CLAUDE.md(symlink,git 可正常追蹤)等 `.md` 交付物**——排除 `orchestrator/` 過程目錄與**執行產物**(`node_modules/`、`dist/`、coverage 等)。執行產物**硬排除、不依賴 `.gitignore`**。若 `git status --porcelain` 出現 `node_modules/` 等路徑,視為 `.gitignore` 缺漏訊號,如實記進「警告」。預設模式「排除一切 `.md`」在 initial 模式**不適用**(README 是交付物不是過程產物)。
...
- **journal 照舊**:若呼叫端提供過程紀錄清單,照樣寫到 `journal/<scope>`(同一個 repo 裡的孤兒分支,永不 merge 回 main)。
- **自檢差異**:「commit 內無任何 `.md`」改為「**commit 內不含 `orchestrator/` 或執行產物**」;README / AGENTS.md / CLAUDE.md(symlink)**應在** commit 內,缺了反而是漏。

## 模式:declared(`commitMode === "declared"`)

前提與預設模式相同(隔離 worktree、分支 `<type>/<scope>`、worktree 從乾淨開始),**差異只在挑檔規則**;未提到的(staged 先擋、分支確認、journal 寫入、硬拒絕處置、local 界線、輸出結構)**全部照預設模式**:

- **挑檔規則(關鍵差異)**:commit 內容 = ① **聲明交付清單**:呼叫端提供的交付檔路徑,排除 `orchestrator/` 前綴後的路徑；② **聯集測試檔**:`git status --porcelain` 中命中 `*.test.*` / `*.spec.*` / `__tests__/` 的變更檔。declared 模式允許聲明交付清單內的 `.md` 交付物；未被聲明且不屬測試檔的變更不收。過程摘要 / 本站自己的 `.md` 摘要是過程紀錄,只寫到 journal,不進交付 commit。
- **聲明檔核對**:聲明交付清單(①)中每個路徑都應存在於 worktree 且在 `git status --porcelain` 有變更紀錄;**聲明了卻無變更 / 不存在** → 硬拒絕(揭露哪個路徑對不上——可能落地站漏做或外部動過碼);反向地,porcelain 裡**有變更卻不在 ①∪②** 的檔(排除 `orchestrator/` 與 `.md` 過程產物)→ 不收,但如實記進「警告」(落地站可能漏聲明,人工確認)。
...
被叫起來時 harness 會給本 skill 的 base directory(形如 `…/skills/auto-commit`),以下用 `$SKILL_DIR`。journal 寫入腳本固定在 `$SKILL_DIR/scripts/journal-publish.js`。
...
3. **檢查 staged(先擋,在挑檔之前)**:`git status --porcelain` 若有**任何**已 staged 的 paths → **硬拒絕並說明**,不做任何後續挑檔 / `git add`(乾淨 worktree 前提下,已 staged 代表外部介入;**先擋下才不會把外部 staged 變更混進候選判斷**)。
...
10. **寫入過程紀錄到 `journal/<scope>`**:將呼叫端提供的**過程紀錄清單**交給腳本——
    ```
    node "$SKILL_DIR/scripts/journal-publish.js" --scope <scope> -- <檔1> <檔2> ...
    ```
    腳本用底層 plumbing 把這些檔提交到孤兒分支 `journal/<scope>`(已存在則接一個新 commit、保留先前紀錄;tree 內**保留相對路徑**如 `<scope>/orchestrator/triage.md`,不同站同名檔不會互相覆蓋),**全程不 checkout、不碰當前 worktree**。解析回傳 JSON 取 `branch` / `commit`。
    > **journal 收檔規則**:`orchestrator/` 不進交付物 commit,但呼叫端提供的過程紀錄清單要寫進 journal 分支作為審計紀錄。
...
- **分支**: <commitType>/<scope>(當前 worktree 分支,由 `worktree-setup` 建立)
...
- staged 檔案數：<n>；寫到 journal 的 md 數：<m>
